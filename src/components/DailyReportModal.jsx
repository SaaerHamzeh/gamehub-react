...
const DailyReportModal = ({ isOpen, onClose }) => {
  const { analytics, sessions, cafeItems, closeDayReport } = useApp();
  ...
  const todaysSessions = sessions.filter(s => s.endTime && new Date(s.endTime).toDateString() === todayStr);
  const totalPlayEarnings = todaysSessions.reduce((sum, s) => sum + (s.totalCost - (s.ordersCost || 0)), 0);
  ...
  const handleCloseDay = async () => {
    ...
    const result = await closeDayReport();
    setLoading(false);
    if (result.success) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[70] flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700 animate-fade-in-up">
        
        {/* Header */}
        <div className="p-8 border-b border-gray-100 dark:border-gray-700 text-center relative">
          <button onClick={handleClose} className="absolute right-6 top-6 text-gray-400 hover:text-red-500 transition"><i className="fas fa-times text-xl"></i></button>
        ...
        {closedReport && (
          <div className="px-8 pt-6">
            <div className="rounded-2xl bg-emerald-500/10 border border-emerald-500/30 p-5 mb-4">
        ...
        {/* Live Analytics */}
        <div className="p-8 grid grid-cols-1 sm:grid-cols-2 gap-6 bg-gray-50/50 dark:bg-gray-900/30">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <p className="text-3xl font-black text-rose-500">{formatMoney(analytics?.completedRevenue || 0)}</p>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-emerald-500/20 shadow-sm">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">صافي الربح (لحظي)</p>
            <p className="text-3xl font-black text-emerald-500">{formatMoney(analytics?.netProfit || 0)}</p>
        ...
        <div className="px-8 pb-4">
          ...
          <div className="max-h-40 overflow-y-auto custom-scrollbar border rounded-xl dark:border-gray-700">
        ...
        {/* Footer Actions */}
        <div className="p-8 border-t border-gray-100 dark:border-gray-700 flex flex-wrap justify-between gap-3">
          ...
          <button
            ...
            className={`px-6 py-3 rounded-2xl font-bold transition flex-1 flex items-center justify-center gap-2
              ${loading ? 'bg-gray-300 dark:bg-gray-700 cursor-not-allowed opacity-60' : 'bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-700 hover:to-emerald-600 text-white shadow-lg active:scale-95'}`}
          >
            {loading
