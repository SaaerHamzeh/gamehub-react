...
const SessionForm = () => {
  const { devices, addSession, isStationActive, permissions, t } = useApp();
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!deviceType || !stationId) { alert(t('select_device_station')); return; }
    ...
    if (isStationActive(stationId)) { alert(`${t('station')} ${stationId} ${t('is_already_occupied')}`); return; }
    ...
    <form onSubmit={handleSubmit} className="mt-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          >
            {stations.map(st => (
              <option key={st} value={st}>{st} {isStationActive(st) ? `🔴 ${t('active')}` : ""}</option>
            ))}
          </select>
        </div>
      </div>
