...
const SessionForm = () => {
  const { devices, addSession, isStationActive } = useApp();
  ...
  const handleSubmit = (e) => {
    ...
    if (pricePerHour <= 0) { alert("Price per hour must be > 0"); return; }
    if (sessionType === 'PRE' && durationHours <= 0) { alert("Duration must be > 0 for prepaid sessions"); return; }
    ...
    addSession({
      ...
      durationHours: sessionType === 'PRE' ? parseFloat(durationHours) : null,
    });
    setName('');
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          <input
            ...
            className="mt-1 w-full px-4 py-2 rounded-xl border focus:ring-rose-500 dark:bg-gray-700 bg-gray-50 dark:border-gray-600 border-gray-300 dark:text-white"
          />
        </div>
        <div>
          ...
          <input
            ...
            className={`mt-1 w-full px-4 py-2 rounded-xl border focus:ring-rose-500 dark:bg-gray-700 bg-gray-50 dark:border-gray-600 border-gray-300 dark:text-white transition-opacity ${sessionType === 'POST' ? 'opacity-50' : ''}`}
          />
        </div>
      </div>
      <button
      ...
      <div className="mt-4 pt-3 text-xs flex justify-between dark:text-gray-400 text-gray-500">
        <span><i className="fas fa-hourglass-half"></i> Auto‑ends after set duration</span>
        <span><i className="fas fa-charging-station"></i> Manual override available</span>
      </div>
    </form>
  );
};
